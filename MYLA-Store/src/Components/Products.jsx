    ...
    <section>
      ...
      {selectedProduct && (
        <div onClick={closeModal} className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4 h-140 sm:h-120 xl:h-150 overflow-y-auto shadow-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl sm:text-2xl font-semibold">{t(selectedProduct.name)}</h2>
              <button
                onClick={closeModal}
          ...
          <div className="flex justify-center">
            <img
              ...
              className="w-80 h-70 sm:w-full sm:h-90 rounded-lg mb-4"
            />
          </div>
